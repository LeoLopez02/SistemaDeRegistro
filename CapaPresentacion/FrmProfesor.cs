...
namespace CapaPresentacion
{
    public partial class FrmProfesor : Form
    {
        ...
        private static FrmProfesor _instancia;
        public static FrmProfesor GetInstancia()
        {
            if (_instancia == null)
            {
                _instancia = new FrmProfesor();
            }
            return _instancia;
        }
        ...
        {
            this.dtListado.DataSource = NegocioProfesor.Mostrar();
            this.OcultarColumna();
        ...
        {
            this.dtListado.DataSource = NegocioProfesor.BuscarNombre(this.txtBuscar.Text);
            this.OcultarColumna();
            lblTotal.Text = "Total de Registros: " + Convert.ToString(dtListado.Rows.Count);
        }
        ...
        {
            this.dtListado.DataSource = NegocioProfesor.BuscarCedula(this.txtBuscar.Text);
            this.OcultarColumna();
            lblTotal.Text = "Total de Registros: " + Convert.ToString(dtListado.Rows.Count);
        }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dtListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            RepuestaEliminar = NegocioProfesor.Eliminar(Convert.ToInt32(Codigo));
        ...
        {
            this.txtId.Text = Convert.ToString(this.dtListado.CurrentRow.Cells["IdProfesor"].Value);
            this.txtNombre.Text = Convert.ToString(this.dtListado.CurrentRow.Cells["Nombre_Profesor"].Value);
            ...
            this.tabProfesor.SelectedIndex = 1;
        }
        ...
        private void btnInsertar_Click(object sender, EventArgs e)
        {
            ...
            {
                string Respuesta = "";
                if (this.txtNombre.Text == string.Empty || this.txtApellido.Text == string.Empty || this.maskedCedula.Text == string.Empty || this.comboGenero.Text == string.Empty || this.txtNacionalidad.Text == string.Empty || this.comboEstatus.Text == string.Empty || this.comboCurso.Text == string.Empty)
                {
                ...
                else
                {
                    if (this.EsNuevo)
                    {
                        Respuesta = NegocioProfesor.Insertar(this.txtNombre.Text.Trim(), this.txtApellido.Text.Trim(), this.maskedTelefono.Text.Trim(), this.txtDireccion.Text.Trim(), this.maskedCedula.Text.Trim(), this.comboGenero.Text.Trim(), this.txtCondicionMedica.Text.Trim(), this.dtFechaNacimiento.Value, this.dtFechaIngreso.Value, this.txtCorreoElectronico.Text.Trim(), this.txtNacionalidad.Text.Trim(), this.txtTitulo.Text.Trim(), this.comboEstatus.Text.Trim(), Convert.ToInt32(this.txtIdCurso.Text));
                    }
                    else
                    {
                        Respuesta = NegocioProfesor.Editar(Convert.ToInt32(this.txtId.Text), this.txtNombre.Text.Trim(), this.txtApellido.Text.Trim(), this.maskedTelefono.Text.Trim(), this.txtDireccion.Text.Trim(), this.maskedCedula.Text.Trim(), this.comboGenero.Text.Trim(), this.txtCondicionMedica.Text.Trim(), this.dtFechaNacimiento.Value, this.dtFechaIngreso.Value, this.txtCorreoElectronico.Text.Trim(), this.txtNacionalidad.Text.Trim(), this.txtTitulo.Text.Trim(), this.comboEstatus.Text.Trim(), Convert.ToInt32(this.txtIdCurso.Text));
                    }
            ...
            {
                MessageBox.Show(ex.Message + ex.StackTrace + " Botón Insertar Profesor.");
            }
        }
