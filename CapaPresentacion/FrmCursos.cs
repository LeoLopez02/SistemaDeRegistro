...
namespace CapaPresentacion
{
    public partial class FrmCursos : Form
    {
        ...
        {
            this.dtListado.DataSource = NegocioCursos.Mostrar();
            this.OcultarColumna();
        ...
        {
            this.dtListado.DataSource = NegocioCursos.BuscarNombre(this.txtBuscar.Text);
            this.OcultarColumna();
            lblTotal.Text = "Total de Registros: " + Convert.ToString(dtListado.Rows.Count);
        }
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dtListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            RepuestaEliminar = NegocioCursos.Eliminar(Convert.ToInt32(Codigo));
        ...
        {
            this.txtId.Text = Convert.ToString(this.dtListado.CurrentRow.Cells["IdCurso"].Value);
            this.txtNombre.Text = Convert.ToString(this.dtListado.CurrentRow.Cells["Nombre"].Value);
            this.tabCursos.SelectedIndex = 1;
        }
        ...
        private void btnInsertar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.EsNuevo)
                    {
                        Respuesta = NegocioCursos.Insertar(this.txtNombre.Text.Trim());
                    }
                    else
                    {
                        Respuesta = NegocioCursos.Editar(Convert.ToInt32(this.txtId.Text.Trim()), this.txtNombre.Text.Trim());
                    }
            ...
            {
                MessageBox.Show(ex.Message + ex.StackTrace + " Botón Insertar Cursos.");
            }
        }
